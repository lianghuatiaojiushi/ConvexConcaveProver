...
凹凸不等式证明器(ConvexConcaveProver).skill 是一个用于证明单变量不等式的小型证明器。它面向一类特定的不等式证明方法：把问题整理成一个凹函数 $L(x)$ 大于一个凸函数 $R(x)$ ，然后在二者之间寻找一条直线作为中间函数。
...
3. 检查 $L(x)$ 是否为凹函数(convex), $R(x)$ 是否为凸函数(concave)。
4. 数值寻找 $L(x)-R(x)$ 的全局最小点 $x^\ast$。
5. 对 $x^\ast$ 做连分数展开，依次取收敛分数 $x_1,x_2,\ldots$ 作为候选切点。
6. 在候选切点 $x_n$ 处作 $R(x)$ 的切线 $h_n(x)$。由于 $R(x)$ 是 concave，自动有 $h_n(x)\ge R(x)$。
7. 检查 $L(x)-h_n(x)$ 的全局最小值是否非负。
8. 若成功，则输出一条形式更美观的证明。
...
line_proof:
  tangent_at = 1/2 ~= 0.5
  exact: h(x) = x + (x + 1/2)/(2*sqrt(1/2)) + 211/481
proof:
  注意到
  e^x>x + (x + 1/2)/(2*sqrt(1/2)) + 211/481>=x + √x + 211/481
  证毕！
```
...
$$
h(x)=\frac{30}{17}x-1+\ln\frac{17}{30}+\frac{261}{112}
$$
...
$$
e^x>\frac{30}{17}x-1+\ln\frac{17}{30}+\frac{261}{112}\ge\ln x+\frac{261}{112}
$$
